...
if (isset($_POST['shairport-sync']) && $_POST['shairport-sync'] != $_SESSION['shairport-sync']){
	// load worker queue 
	// start / respawn session
	session_start();
	// save new value on SQLite datastore
	if ($_POST['shairport-sync'] == 1 OR $_POST['shairport-sync'] == 0) {
	playerSession('write',$db,'shairport-sync',$_POST['shairport-sync']);
	}
	// set UI notify
	if ($_POST['shairport-sync'] == 1) {
...
$_system_select['shairport-sync1'] .= "<input type=\"radio\" name=\"shairport-sync\" id=\"toggleshairport-sync1\" value=\"1\" ".(($_SESSION['shairport-sync'] == 1) ? "checked=\"checked\"" : "").">\n";
$_system_select['shairport-sync0'] .= "<input type=\"radio\" name=\"shairport-sync\" id=\"toggleshairport-sync2\" value=\"0\" ".(($_SESSION['shairport-sync'] == 0) ? "checked=\"checked\"" : "").">\n";
